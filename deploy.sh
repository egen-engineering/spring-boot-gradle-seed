...
projectName=build-api-marketplace #spring-boot
...
function run(){
   ...
   #echo "java -jar $destFile --server.port=$serverPort" | at now + 1 minutes

   nohup nice java -jar -Dspring.profiles.active=prod $destFile --server.port=$serverPort $> $dstLogFile 2>&1 &

   echo "COMMAND: nohup nice java -jar -Dspring.profiles.active=prod $destFile  --server.port=$serverPort $> $dstLogFile 2>&1 &"

    echo " "
}
function changeFilePermission(){

    echo "Changing File Permission: chmod 764 $destFile"
